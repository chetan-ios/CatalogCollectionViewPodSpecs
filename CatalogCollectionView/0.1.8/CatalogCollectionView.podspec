Pod::Spec.new do |s|

# 1
s.platform = :ios
s.ios.deployment_target = '10.0'
s.name = "CatalogCollectionView"
s.summary = "CatalogCollectionView lets you ."
s.requires_arc = true

# 2
s.version = "0.1.8"

# 3
s.license = { :type => "MIT", :file => "LICENSE" }

# 4 - Replace with your name and e-mail address
s.author = { "Chetan Kumar B V" => "chetan6chetan@gmail.com" }

# 5 - Replace this URL with your own GitHub page's URL (from the address bar)
s.homepage = "https://github.com/chetan-ios/CatalogCollectionView"

# 6 - Replace this URL with your own Git URL from "Quick Setup"
s.source = { :git => "https://github.com/chetan-ios/CatalogCollectionView.git",
:tag => "#{s.version}" }

# 7
s.framework = "UIKit"

# 8
s.source_files = "CatalogCollectionView/**/*.{swift}"

# 9
s.resources = "CatalogCollectionView/**/*.{png,jpeg,jpg,storyboard,xib,xcassets}"

# 10
s.swift_version = "4.2"

end
